...
import { properties, cities, formatINR } from "@/utils/properties";
...
const categories = [
  {
    ...
    image: catBuy,
    match: (p) => p.type === "Villa" || p.type === "Apartment",
    subs: [
      { id: "villa", label: "Villas", match: (p) => p.type === "Villa" },
      { id: "apartment", label: "Apartments", match: (p) => p.type === "Apartment" },
      { id: "furnished", label: "Furnished", match: (p) => !!p.furnished },
      { id: "ready", label: "Ready to Move", match: (p) => !!p.readyToMove },
    ],
  },
  {
    id: "plots",
    ...
    match: (p) => p.type === "Plot",
    subs: [
      { id: "all", label: "All Plots", match: (p) => p.type === "Plot" },
      { id: "mountain", label: "Mountain View", match: (p) => p.amenities.some((a) => /mountain/i.test(a)) },
    ],
  },
  {
    id: "commercial",
    ...
    image: catCommercial,
    match: (p) => p.type === "Commercial" || p.purpose === "Commercial",
    subs: [
      { id: "all", label: "All Commercial", match: (p) => p.type === "Commercial" },
      { id: "showroom", label: "Showrooms", match: (p) => /showroom|glass/i.test(p.title) },
    ],
  },
  {
    id: "rent",
    ...
    subs: [
      ...
      { id: "furnished", label: "Furnished", match: (p) => p.purpose === "Rent" && !!p.furnished },
    ],
  },
  {
    id: "luxury",
    ...
    image: heroImg,
    match: (p) => p.price >= 20000000 || /heritage|chalet|wooden/i.test(p.title),
    subs: [
      { id: "all", label: "All Luxury", match: (p) => p.price >= 20000000 },
      { id: "chalet", label: "Chalets", match: (p) => /chalet|heritage/i.test(p.title) },
    ],
  },
];
...
export default function ExplorePage() {
  ...
  const stats = useMemo(() => {
    ...
    const avg = Math.round(properties.reduce((a, b) => a + b.price, 0) / total);
  ...
  const cityStats = useMemo(
    () =>
      cities.map((c) => ({
        ...c,
        count: properties.filter((p) => p.city === c.name).length,
      })),
    []
  );

  const trending = useMemo(() => [...properties].sort((a, b) => b.price - a.price).slice(0, 4), []);
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-border bg-background/80 px-3 py-1 text-xs font-medium text-muted-foreground backdrop-blur">
                <Sparkles className="h-3.5 w-3.5 text-primary" /> Explore J&K Real Estate
              </div>
      ...
      <section className="container-px mx-auto max-w-7xl py-10">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
          {categories.map((c) => {
            ...
            const count = properties.filter(c.match).length;
            return (
              <button
                key={c.id}
        ...
        {active && (
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            >
              All ({properties.filter(active.match).length})
            </button>
            {active.subs.map((s) => {
              const cnt = properties.filter((p) => active.match(p) && s.match(p)).length;
              const sel = sub === s.id;
              return (
                <button
                  key={s.id}
      ...
      <section ref={resultsRef} className="container-px mx-auto max-w-7xl pb-10 pt-2 scroll-mt-20">
        <div className="flex items-end justify-between">
          ...
          {cart.length > 0 && (
            ...
            >
              Compare {cart.length} <ArrowUpRight className="h-3.5 w-3.5" />
            </Link>
          )}
        </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
            {filtered.map((p) => (
              <div key={p.id} className="relative">
                ...
                >
                  {cart.includes(p.id) ? "✓ In Compare" : "+ Compare"}
                </button>
              </div>
            ))}
          </div>
